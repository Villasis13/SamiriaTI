...
*/
try{
    $logo = EscposImage::load("media/logo/samiria_logo.jpg", false);
...
$total = 0;
if($venta->cambiar_concepto==2){
    /*Alinear a la izquierda para la cantidad y el nombre*/
    $printer->setJustification(Printer::JUSTIFY_LEFT);
    $printer->text($venta->concepto_nuevo . "\n");

    /*Y a la derecha para el importe*/
    $printer->setJustification(Printer::JUSTIFY_CENTER);
    $printer->text(1 . "      x      " .$venta->venta_total.'      S/ ' . $venta->venta_total . "\n");
    ...
    foreach ($detalle_venta as $dp) {
        ...
        /*Y a la derecha para el importe*/
        $printer->setJustification(Printer::JUSTIFY_CENTER);
        $printer->text($dp->venta_detalle_cantidad . "      x      " . number_format($dp->venta_detalle_valor_unitario, 2) . '      S/ ' . number_format($dp->venta_detalle_valor_total, 2) . "\n");
    }
}
/*
...
if($venta->venta_totaldescuento > 0){
    $printer->text("                           DESCUENTO ". $venta->venta_descuento_global ."\n");
