...
class Receta{
    ...
    public function listar_unidad_precio($id_recurso_sede){
        try{
            $sql = 'select * from recursos_sede rs inner join medida um on rs.id_medida = um.id_medida where id_recurso_sede = ? limit 1';
            $stm = $this->pdo->prepare($sql);
    ...
    public function guardar_receta($model){
        ...
        try{
            if(isset($model->id_receta)){
                ...
                $sql = 'insert into recetas (id_usuario, receta_nombre, receta_fecha, receta_estado,receta_codigo) values (?,?,?,?,?)';
                $stm = $this->pdo->prepare($sql);
                $stm->execute([
                    $model->id_usuario,
                    $model->receta_nombre,
    ...
    public function listar_detalle_recetas($id){
        try{
            $sql = 'select * from recetas r inner join detalle_recetas dr on r.id_receta = dr.id_receta inner join recursos_sede rs on dr.id_recursos_sede = rs.id_recurso_sede 
                    inner join recursos re on rs.id_recurso = re.id_recurso inner join medida um on dr.detalle_receta_unidad_medida = um.id_medida where r.id_receta = ?';
            $stm = $this->pdo->prepare($sql);
            $stm->execute([$id]);
            return $stm->fetchAll();
        } catch (Throwable $e){
    ...
    public function listar_detalle_recetas_conversion($id){
        try{
            $sql = 'select * from detalle_recetas dc inner join conversiones c on dc.detalle_receta_unidad_medida = c.id_conversion inner join medida um on c.conversion_unidad_medida = um.id_medida where dc.id_detalle_receta = ?';
            $stm = $this->pdo->prepare($sql);
            $stm->execute([$id]);
            return $stm->fetch();
        } catch (Throwable $e){
    ...
    public function listar_sub_recetas($id){
        try{
            $sql = 'select * from sub_recetas sr inner join recetas r on sr.id_receta_2 = r.id_receta inner join medida um on sr.id_medida = um.id_medida where sr.id_receta = ?';
            $stm = $this->pdo->prepare($sql);
            $stm->execute([$id]);
            return $stm->fetchAll();
        } catch (Throwable $e){
    ...
    public function buscar_medida($id){
        try{
            $sql = 'select rs.id_medida from recursos_sede rs inner join medida m on rs.id_medida = m.id_medida where id_recurso_sede = ?';
            $stm = $this->pdo->prepare($sql);
            $stm->execute([$id]);
            return $stm->fetch();
        } catch (Throwable $e){
