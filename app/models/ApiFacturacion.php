...
class ApiFacturacion
{
    public function EnviarComprobanteElectronico($emisor, $nombre, $rutacertificado, $ruta_archivo_xml, $ruta_archivo_cdr,$id_venta)
    {
        ...
        $ruta_firma = $rutacertificado. 'certificado_20607969877.pfx'; //ruta del archivo del certicado para firmar
        $pass_firma = 'sAm1r1AbUfe00';
        ...
        if($result == 1){
            ...
            if($result == 1){
                ...
                if($httpcode == 200)//200: La comunicacion fue satisfactoria
                {
                    ...
                    $codigo = $doc->getElementsByTagName('faultcode')->item(0)->nodeValue;
                    $mensaje = $doc->getElementsByTagName('faultstring')->item(0)->nodeValue;
                    $estado_sunat = 'Hubo o existe un problema de conexión: ' . $codigo . ' Msje:' . $mensaje;
