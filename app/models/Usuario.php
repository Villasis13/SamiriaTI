...
class Usuario{
    ...
    public function listar_usuarios_admin(){
        try{
            $sql = 'select * from usuarios u inner join personas p on u.id_persona = p.id_persona inner join roles r on u.id_rol = r.id_rol where r.id_rol=3 and u.usuario_estado = 1';
            $stm = $this->pdo->prepare($sql);
            $stm->execute();
            return $stm->fetchAll();
        } catch (Throwable $e){
