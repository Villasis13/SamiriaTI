...
class GeneradorXML
{
    function CrearXMLFactura($nombrexml, $emisor, $cliente, $comprobante, $detalle, $cuotas)
    {
        ...
        $num = new Nmletras();
        $total_letras = $num->num2letras($comprobante->venta_total);
        ...
        */
        ...
        foreach($detalle as $v){
            ...
            $xml.='<cac:InvoiceLine>
               ...
               <cbc:LineExtensionAmount currencyID="'.$comprobante->abrstandar.'">'.round($v->venta_detalle_valor_total,2).'</cbc:LineExtensionAmount>
            ...
            $xml.= '<cac:TaxTotal>
                      <cbc:TaxAmount currencyID="'.$comprobante->abrstandar.'">'.number_format($v->venta_detalle_total_igv, 2).'</cbc:TaxAmount>
                      <cac:TaxSubtotal>
    ...
    function CrearXMLNotaCredito($nombrexml, $emisor, $cliente, $comprobante, $detalle, $descripcion_nota,$cuotas)
    {
        $doc = new DOMDocument();
        $doc->formatOutput = FALSE;
        $doc->preserveWhiteSpace = TRUE;
        ...
        /*if($comprobante->id_tipo_pago != "5"){
            ...
            foreach ($cuotas as $c){
                ...
                $xml.= '<cac:PaymentTerms>
                        ...
                        <cbc:PaymentMeansID>Cuota'. str_pad($numero, 3, "00", STR_PAD_LEFT).'</cbc:PaymentMeansID>
                        <cbc:Amount currencyID="PEN">'.$c->venta_cuota_importe.'</cbc:Amount>
                        <cbc:PaymentDueDate>'.date('Y-m-d', strtotime($c->venta_cuota_importe)).'</cbc:PaymentDueDate>
                    </cac:PaymentTerms>';
            }
        }*/
    ...
    function CrearXMLNotaDebito($nombrexml, $emisor, $cliente, $comprobante, $detalle, $descripcion_nota,$cuotas)
    {
