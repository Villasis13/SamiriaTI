...
function calcular_total(id){
    ...
    $('#venta_total_antiguo').val(total_pedido_detalle.toFixed(2));
...
function agregar(){
    ...
    let descuentito  = $("#des_global_").val();
    let venta_total_antiguo  = $("#venta_total_antiguo").val();
    let id_boleta_pis  = $("#id_boleta_pis").val();
    let correlativo_pis  = $("#correlativo_pis").val();
    var des_global_ = $('#des_global_').val();
    //FIN DE LOS VALORES
    //INICIO
    var cambiar_concepto = $('#cambiar_concepto').val();
    var concepto_nuevo = $('#concepto_nuevo').val();
    var id_comanda = $('#id_comanda').val();
    var tipo_desct_v = $('#tipo_desct_v').val();
    //FIN
    if(partir_pago == 1){
    ...
    if(valor){
        var cadena = "id_cliente=" + id_cliente +
            ...
            "&id_mesa=" + id_mesa+
            "&descuentito=" + descuentito+
            "&venta_total_antiguo=" + venta_total_antiguo+
            "&id_boleta_pis=" + id_boleta_pis+
            "&correlativo_pis=" + correlativo_pis +
            "&des_global_=" + des_global_ +
            "&cambiar_concepto=" + cambiar_concepto +
            "&concepto_nuevo=" + concepto_nuevo +
            "&id_comanda=" + id_comanda +
            "&tipo_desct_v=" + tipo_desct_v;
        $.ajax({
            type: "POST",
...
function convertir(){
    ...
    var id_mesa = $('#id_mesa').val();
    var id_comanda = $('#id_comanda').val();
    var correlativo_pis = $('#correlativo_pis').val();
    var id_boleta_pis = $('#id_boleta_pis').val();

    if(valor){
        var cadena = "datos_detalle_pedido=" + datos_detalle_pedido +
        "&id_mesa=" + id_mesa +
        "&id_comanda=" + id_comanda +
        "&correlativo_pis=" + correlativo_pis +
        "&id_boleta_pis=" + id_boleta_pis;
        $.ajax({
            type: "POST",
            url: urlweb + "api/Pedido/actualizar_venta_coman",
            data: cadena,
            dataType: 'json',
            beforeSend: function () {
                cambiar_estado_boton(boton, 'cobrando...', true);
            ...
            success: function (r) {
                ...
                switch (r.result.code) {
                    case 1:
                        respuesta('¡Operación realizada correctamente!', 'success');
                        if(r.result.pago == 0){
                            setTimeout(function () {
                                location.reload()
                            }, 1000);
                        ...
                        break;
                    case 2:
                        respuesta('Error al realizar operación', 'error');
                        break;
                    case 5:
                        respuesta('Error al realizar operación, revisar Cliente', 'error');
                        break;
                    default:
                        respuesta('¡Algo catastrofico ha ocurrido!', 'error');
                        break;
...
function guardar_pedido(id,producto_nombre, precio,stock,afectacion){
    $('#id_producto').val(id);
    $('#producto_nombre').html(producto_nombre);
    $('#comanda_detalle_precio').val(precio);
    $('#stock_escondido').val(stock);
    $('#codigo_afectacion_d').val(afectacion);
    $.ajax({
        type: "POST",
        url: urlweb + "api/Pedido/jalar_opciones_",
        data: "id="+id,
        dataType: 'json',
        success:function (r) {
            $("#opciones").html(r.opciones_);
        }
    });
}
...
function guardar_pedido_nuevo(id,producto_nombre, precio,stock){
    $('#id_producto').val(id);
    ...
    $.ajax({
        ...
        dataType: 'json',
        success:function (r) {
            $("#opciones_dentro").html(r.opciones_);
        }
    });
...
function eliminar_comanda_detalle(){
    ...
    // valor = validar_campo_vacio('id_comanda_detalle',id_comanda_detalle, valor);
    // valor = validar_campo_vacio('id_comanda',id_comanda, valor);
    // valor = validar_campo_vacio('id_mesa',id_mesa, valor);
    valor = validar_campo_vacio('password',password, valor);
    valor = validar_campo_vacio('comanda_detalle_eliminacion',comanda_detalle_eliminacion, valor);
    //var password = $('#password').val();
...
function transferir_mesa_x_pedido(){
    ...
    var id_mesa = $('#id_mesa').val();
    var password_xp = $('#password_xp').val();

    valor = validar_campo_vacio('password_xp', password_xp, valor);

    if(valor) {
        var cadena = "id_mesa_transp=" + id_mesa_transp +
            "&id_comanda_detalle_transferir=" + id_comanda_detalle_transferir +
            "&id_mesa=" + id_mesa +
            "&password_xp=" + password_xp;
        $.ajax({
            type: "POST",
            ...
            success: function (r) {
                switch (r.result.code) {
                    ...
                    case 5:
                        respuesta("La Contraseña de Usuario no es Correcta", 'error');
                        $('#password_xp').css('border','solid red');
                        break;
                    default:
                        respuesta('¡Algo catastrofico ha ocurrido!', 'error');
                        break;
...
//funcion para borrar all
function eliminar_todo_pedido(){
    var valor = true;
    //var password = "";
    var password_todo = $('#password_todo').val();
    var id_comanda_todo = $('#id_comanda_todo').val();
    var id_mesa_todo = $('#id_mesa_todo').val();
    var motivo_eliminar_todo = $('#motivo_eliminar_todo').val();
    //Validamos si los campos a usar no se encuentran vacios
    valor = validar_parametro_vacio('id_comanda_todo',id_comanda_todo, valor);
    //var password = $('#password').val();

    if(valor) {
        var cadena = "id_mesa_todo=" + id_mesa_todo +
            "&id_comanda_todo=" + id_comanda_todo +
            "&motivo_eliminar_todo=" + motivo_eliminar_todo +
            "&password_todo=" + password_todo;
        $.ajax({
            type: "POST",
            url: urlweb + "api/Pedido/eliminar_todo_pedido",
            data: cadena,
            dataType: 'json',
            success: function (r) {
                switch (r.result.code) {
                    case 1:
                        ...
                        break;
                    case 2:
                        respuesta('Error al eliminar pedidos, comuniquese con BufeoTec', 'error');
                        break;
                    case 5:
                        respuesta("La Contraseña del Usuario no es Correcta", 'error');
                        $('#password_todo').css('border','solid red');
                        break;
                    default:
                        respuesta('¡Algo catastrofico ha ocurrido!', 'error');
                        break;
